...
public class ILCodeLoader : ModSystem
{
    public override void Load()
    {
        ...
        TryCatch(() => IL_Projectile.AI_061_FishingBobber_GiveItemToPlayer += IL_Projectile_AI_061_FishingBobber_GiveItemToPlayer, nameof(IL_Projectile_AI_061_FishingBobber_GiveItemToPlayer));
    }

    private void IL_Projectile_AI_061_FishingBobber_GiveItemToPlayer(ILContext il)
    {
        TryCatch(() =>
        {
            ILCursor c = new(il);
            ...
            c = new(il);
            c.EmitDelegate(() => ConfigContent.EnableMod);
            c.Emit(OpCodes.Brtrue, label);
        }, nameof(IL_Projectile.AI_061_FishingBobber_GiveItemToPlayer));
    }
...
#pragma warning restore CS0618 // 类型或成员已过时
    ...
    private void IL_ItemSlot_Draw_SpriteBatch_ItemArray_int_int_Vector2_Color(ILContext il)
    {
        TryCatch(() =>
        {
            const byte index = 31; // int32 num10
            ILCursor c = new(il);
    ...
    private static int CountBaits(int source, Player player)
    {
        ...
        return AutoFisherUtils.CountBait(player,
            true,
    ...
    {
        // 当该选项启用时，总是视为没有任务鱼
        if (ConfigContent.EnableMod && ConfigContent.Server.Common.FishingQuests.CanCatchQuestFishWhenSameOneIsInInventory)
            return false;
    ...
    private static int DropItem(int itemDrop, Projectile bobber, FishingAttempt attempt)
    {
        ...
        {
            FishingCatchesCalculator.Catches.AddOrUpdate(itemDrop, 1, static (_, oldValue) => oldValue + 1);
            return 0;
        }
        ...
        // 如果渔线断裂或渔获在有声纳buff的情况下被过滤，则本次不会钓起任何渔获
        ...
        if (info.catchSuccessfully)
        {
            ...
            // 未被过滤且未被卖出，则尝试自动打开
            if (!info.filtered && !info.autoSold)
            {
        ...
        if (info.filtered && !info.autoSold && player.sonarPotion)
            info.consumedBait = false;
        else
    ...
    private static bool DropItem_AutoSell(Player player, Item item, ref CatchesInfo info)
    {
        ...
        {
            AutoFisherUtils.SellItem(player, item, out var value);
            info.coins = value;
            item.TurnToAir();
            return true;
        }
    ...
    private int SpawnNPC(int npcSpawn, Projectile bobber, FishingAttempt attempt)
    {
